...
namespace com.wudada.web.mail
{
    public class WuDADAMailService
    {
        ...
        public void SendMail_ToLoginUser_ResetPass(string loginUserId)
        {
            try
            {
                LoginUser loginUser = myService.DaoGetVOById<LoginUser>(loginUserId);
        ...
        private string GetMailBodyStr_LoginUser_ResetPass(LoginUser loginUser)
        {
            ...
            //重設密碼
            string newPass = PasswordUtil.GetRandomPass(5, 8);
            loginUser.Password = EncryptUtil.GetMD5(newPass);
            myService.DaoUpdate(loginUser);
